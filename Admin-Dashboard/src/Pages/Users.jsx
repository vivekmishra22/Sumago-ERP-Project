...
//      if (editingId) {
//       axios
//         .put(`http://localhost:8000/Update/${editingId}`, newData,
...
//     } else {
//       axios
//         .post("http://localhost:8000/add", newData,
...
const Users = () => {
  ...
  const handlePdf = () => {
    ...
    doc.autoTable({
      head: [["Sr.No", "First Name", "Last Name", "Email ID"]],
      body: userData.map((a, index) => [index + 1, a.fname, a.lname, a.email]),
      startY: 30,
    });
    doc.save("User-data.pdf");
  };
  ...
  return (
    <Container className="d-flex justify-content-end">
      <Row className="d-flex justify-content-center mt-2 pt-5">
        ...
        <Modal show={show} onHide={handleClose}>
          <Modal.Header closeButton>
            <Modal.Title>{editingId ? "Update User" : "Add User"}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Form onSubmit={handleSubmit}>
              <Row>
